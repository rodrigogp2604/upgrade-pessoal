...
| D34 | Manifesto do APK | `/api/app/latest` só anuncia versão se o **arquivo existir** no disco; o script valida sha256 e tamanho antes de escrever o manifesto | manifesto apontando para arquivo que sumiu faria o celular baixar um 404 e o painel mostrar QR quebrado. Testado: apagar o .apk faz o servidor voltar a dizer "nada publicado" |
| D32 | Miniatura da prova | a prova é gravada em **dois arquivos**: o cheio (que sobe e é apagado) e uma miniatura de 256px que nunca sai do aparelho; ao receber a linha do servidor, a miniatura é **herdada** pela linha nova | sem isso a prova só apareceria com o PC ao alcance (a `url` do servidor exige rede). Custa poucos KB por prova e mantém o histórico visível offline |
| D33 | Seam de plataforma no upload | `src/sync/filepart.ts` isola *como* um arquivo entra no FormData (`{uri,name,type}` no RN, `Blob` em Node) | é a única parte do upload que não roda fora do aparelho; isolando-a, o teste sobe a prova de verdade para o servidor real em vez de simular |
| D31 | Pull x conflito aberto | linha com conflito pendente **não é sobrescrita pelo pull** (e o personagem congela enquanto houver conflito de missão) | descoberto no teste: sem isso, ao detectar o conflito o pull trazia a versão do PC e a missão que você concluiu no ônibus voltava a "pendente" na tela — a promessa "nada é sobrescrito sem você ver" valia para o banco do PC, mas não para o que você estava olhando. O personagem entra junto porque missão concluída sem o XP correspondente é uma conta visivelmente errada |
| D29 | Operação recusada pelo servidor | o app **desfaz o que criou** (chefão/pagamento inventado offline) e força **pull completo** naquela rodada | descoberto pelo teste de ponta a ponta: com recusa por `week_closed`, o pull incremental não traz a missão de volta (no PC nada mudou, o `updatedAt` continua velho) — a missão ficaria "concluída" no celular para sempre, com XP inflado em silêncio |
| D30 | Dependência entre operações | push em até **3 passes** por rodada; operação cujo id provisório ainda não foi resolvido espera o passe seguinte | "criei o chefão e paguei ele antes de sincronizar" precisa que o `debt.create` volte com o id real antes do `payment.create` sair |
| D26 | Missão principal x secundária | **nova coluna `Mission.kind`** no servidor (`main` \| `side`, default `main`), exposta no pull e nas skills | o protótipo mobile tem a seção "SECUNDÁRIAS · XP BÔNUS", e o servidor não tinha como distinguir. A alternativa era adivinhar por heurística (ex.: "as duas últimas são bônus"), o que quebraria assim que um arco tivesse outro formato. Quem marca é o cowork ao montar o arco |
| D27 | Linhas criadas no celular | id **negativo decrescente** (`-1`, `-2`, …) até o pull trazer a linha real | pagamento e chefão criados offline precisam de id local que nunca colida com os ids do servidor (positivos) e que seja óbvio de reconhecer |
| D28 | Testes do banco do app | `mobile/test/` roda o SQL real de `src/db/*` no **SQLite embutido do Node**, com um adaptador que imita a interface do expo-sqlite e um substituto para `expo-crypto` (via `paths` do `tsconfig.test.json`) | dá para testar migração, cascata, XP e fila sem emulador nem aparelho — e sem duplicar o SQL num arquivo de teste, que é o jeito de o teste passar enquanto o app quebra |
| D23 | Barra de abas | **barra em JS desenhada à mão** (`Tabs` com `tabBar` próprio), não a `NativeTabs` do template | a barra nativa não dá controle de cor, peso e espaçamento — e a do protótipo é parte da identidade |
| D24 | Resolução de módulos do Metro | `resolver.nodeModulesPaths` fixo em `mobile/node_modules` + `disableHierarchicalLookup` | `mobile/` está fora dos workspaces, então existe um segundo `node_modules` na raiz com **React 18** (do painel). Sem travar a busca, o Metro pode misturar dois Reacts e isso aparece só em runtime como "Invalid hook call" |
| D25 | Gradiente de fundo | três faixas de cor empilhadas em vez de `expo-linear-gradient` | mesma sensação do papel quente do protótipo sem somar dependência nativa |
| D22 | Avatar | vira Setting `avatar` (data URL), **reduzido para 256px JPEG q0.85** no painel antes de subir; limite do `express.json` foi para **2mb** | tinha que sair do localStorage para o celular ver a foto; foto de câmera crua em base64 estouraria o parser (padrão 100kb) e engordaria todo pull |
...
| ~~**3. Painel + infra**~~ ✅ | tela "Parear celular" com QR (`qrcode.react`), IP editável e lista de aparelhos; `scripts/lan-ip.ps1` + injeção de `HOST_LAN_IP` no `start.ps1`/`rebuild.ps1` e no `docker-compose.yml`; avatar migrado de localStorage para Setting (com redução para 256px) e `extras` nascendo com uuid | `web/src/components/panels/PairPanel.tsx`, `web/src/lib/image.ts`, `scripts/lan-ip.ps1`, `docker-compose.yml` |
| ~~**4. App — esqueleto**~~ ✅ | Expo SDK 57 (React 19.2 / RN 0.86) em `mobile/`, NativeWind v4 com os tokens do protótipo em variáveis CSS, Rajdhani + IBM Plex Sans, **5 abas com barra desenhada à mão**, tema claro/escuro no interruptor, radar hexagonal e anel de XP em `react-native-svg`, XP flutuante / toast / tela de nível novo. Telas alimentadas por mock com a forma exata do `pull` — a Fase 5 troca a fonte sem tocar em componente | `mobile/src/app/(tabs)/*`, `mobile/src/components/*`, `mobile/src/game/*` |
| ~~**5. App — banco local**~~ ✅ | schema SQLite (espelho + outbox/conflicts/pending_files) com migração por `user_version` e FKs em cascata, repositórios de leitura, mutações que aplicam local **e** enfileiram na mesma transação, tema persistido, estado vazio real + seed só em `__DEV__`. **41 testes** rodando o SQL de verdade fora do aparelho (`npm run test:db`) | `mobile/src/db/*`, `mobile/test/*` |
| ~~**6. App — sync**~~ ✅ | pareamento por QR (câmera) com IP manual e varredura da rede, motor `ping → push (em passes) → pull`, remapeamento de id provisório, chip de status no header, painel de sincronização, auto-sync ao voltar ao app e a cada 5 min. **32 testes de ponta a ponta** com o motor real falando com o servidor real (`npm run test:sync`) | `mobile/src/sync/*`, `mobile/src/app/parear.tsx`, `mobile/test/sync.test.mts` |
| ~~**7. App — conflitos**~~ ✅ | tela de resolução com escolha por registro, "usar tudo do celular/PC" e confirmação em lote; `force` com opId derivado; "usar PC" descarta e puxa a verdade inteira. **Registro em conflito fica congelado no pull** (D31) | `mobile/src/app/conflitos.tsx`, `mobile/src/db/conflicts.ts` |
| ~~**8. App — provas**~~ ✅ | câmera, galeria e arquivo qualquer; imagem reduzida para 1920px/JPEG 85% + **miniatura de 256px que fica no aparelho para sempre**; fila `pending_files`, upload multipart idempotente entre push e pull, original apagado após confirmação, remoção de prova (local ou no PC). **53 testes E2E**, incluindo os bytes chegando inteiros no disco do servidor | `mobile/src/proofs/*`, `mobile/src/db/proofs.ts`, `mobile/src/components/ProofSheet.tsx` |
| ~~**9. Build & distribuição**~~ ✅ | ícones gerados de `assets/torre.svg` (a mesma arte do atalho), `eas.json` perfil `preview` → APK, `scripts/build-apk.ps1` (build na nuvem → `data/apk/` → manifesto com sha256), `/api/app/latest` e `/api/app/download`, QR "Instalar no celular" no painel, aviso de versão nova dentro do app. **11 testes** das rotas de distribuição. ⚠️ o build em si depende de conta Expo e não foi executado aqui | `scripts/build-apk.ps1`, `api/src/routes/app.ts`, `mobile/eas.json`, `mobile/scripts/gerar-icones.mjs` |
| ~~**10. Docs**~~ ✅ | `docs/SYNC.md` (contrato do protocolo), seção "O celular, sem nuvem no meio" no README, instalação e keystore no SETUP. **Pendente: screenshots do app** — dependem do APK instalado num aparelho | `docs/SYNC.md`, `README.md`, `docs/SETUP.md` |
